...
### log files (generated by `ohsnap_run_cluster`)

When an OHSNAP project is run with the `ohsnap_run_cluster` command, a log directory is created containing the standard output/error for each job run. These files can be useful if you encounter an error during a run and should be the first point of enquiry if a run fails. The structure of the log directory is:

```
ls logs/codeml/CladeModelC/CBX1/
c_prune.log  myotis  nmr

ls logs/codeml/CladeModelC/CBX1/myotis/
c_lbl_species_tree.log  c_mk_ctl.log

ls logs/codeml/M2aRel/CBX1/
c_mk_ctl.log  c_prune.log
```

All log files are generated in the `logs/codeml` directory. Here we have an example of the locations of the log files generated for both labelled and unlabelled models (CladeModelC and M2aRel, respectively). In the case of an unlabelled model, all log files are located in `logs/codeml/<model_name>/<gene_name>/`. For labelled models, the output of the `prune_species_tree` job is located in `logs/codeml/<model_name>/<gene_name>`, as this job is only executed once per model but as the rest of the jobs are executed once per set of species groups/labels, there will be a set of log files for each species group/label and these are located in `logs/codeml/<model_name>/<gene_name>/<species_group_name>/`.

### Run directories

Each CodeML run directory is located under `<model_name>/<gene_name>/<species_group_name>/` for labelled models and `<model_name>/<gene_name>/` for unlabelled models, and these directories will contain all used inputs for each run. Additionally, for labelled models, the pruned (unlabelled) tree will be located under `<model_name>/<gene_name>`.

### CodeML output

For convenience, all CodeML output files are copied to a directory called `output` upon run completion. The output files are named according to `<gene_name>.<model_name>.out` and in the case of labelled models, are located in a directory named after each species group.

## Performing a dry run

You can see exactly what steps will be executed in a project using the `ohsnap_check` command. It takes the path to an OHSNAP project directory as an argument, or if none is given, it assumes the current directory is an OHSNAP project directory. If a supplied path or current directory is not an OHSNAP project directory, the `ohsnap_check` command will exit with an **'Error: Snakefile "Snakefile" not present.'** error message. Here is an example of its output (with omissions for space reasons):
...
The `ohsnap_run_cluster` command requires a batch execution system, such as, Open Grid Engine, PBS, Torque etc. The command executes an OHSNAP project on a compute cluster and optionally accepts the path to an OHSNAP project directory. If none is given, the current directory is checked. If a supplied path or current directory is not an OHSNAP project directory, the `ohsnap_run_cluster` command will exit with an **'Error: Snakefile "Snakefile" not present.'** error message. The maximum number of jobs submitted to the execution queue at any one time is specified by the `--num_jobs` option, the default is 10. Jobs are submitted to the batch execution system using the command specified by the `--cluster_cmd` option, the default is:
...
**Important Note:** If the `ohsnap_run_cluster` command is killed or stopped, submitted jobs will continue to run or be queued. You will need to delete/remove them using an appropriate command for your execution environment e.g. `qdel`. 
